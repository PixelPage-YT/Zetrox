...
export async function emojiquiz(
    ...
    try {
        if (i.member) {
            if (i.guild) {
                const emojis =
                    JSON.parse(Deno.readTextFileSync('./data/emojiquiz.json'))
                        .data;
                    ...
                    embeds: [
                        {
                            ...
                            'description':
                                '*Beim Emojiquiz musst du anhand von Emojis herausfinden, um welchen Gegenstand/Film/Song/Person es sich handelt!*\nBenutze `/guess emojiquiz <wort>`, um ein Wort zu raten oder benutze `/guess emojiquiz abbrechen`, um abzubrechen.\n__**Emojis:**__ **' +
                                solution.emojis + '**',
                            'color': 44469,
                            'footer': {
                        ...
                        if (guessed) {
                            if (guessed == 'abbrechen') {
                                await i.channel?.send({
                                    content:
                                        '<:icons_Wrong:947468536492752906> Abgebrochen. <:icons_Wrong:947468536492752906>',
                                });
                                break;
                            }
                            if (
                                solution.solutions.findIndex((index) =>
                                    index.toLowerCase() ===
                                        guessed.toLocaleLowerCase()
                                ) == -1
                            ) {
                            ...
                            if (
                                ...
                                const belohnung: number = solution.points;
                                await answer.respond({
                                    embeds: [
                                        {
                                            'title':
