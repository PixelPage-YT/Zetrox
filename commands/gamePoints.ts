...
export async function gamePoints(i: harmony.Interaction) {
    try {
        if (i.isApplicationCommand()) {
            ...
            if (member) {
                if (i.guild) {
                    if (i.member) {
                        const messagecount = getGamePoints(member);
                        await i.respond({
                            embeds: [{
                                'title': member.user.username,
                                'description':
                                    `Dieser Benutzer hat ${messagecount} GamePoints <:ZetroCoin:935256569984208917>.`,
                                'color': 15658734,
                                'author': {
                                    'name': 'GamePoints',
                                    'icon_url':
