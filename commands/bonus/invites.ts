...
export async function bonusAddInvites(i: harmony.Interaction) {
    try {
        if (i.member) {
                    ...
                    if (anzahl < 10000 && anzahl > 0) {
                        ...
                        await i.respond({
                            embeds: [{
                                ...
                                'description':
                                    `Du hast ${member.user.username} **${anzahl}** Einladungen hinzugefügt.`,
                                'color': 15658734,
                                'author': {
                        ...
                        await i.respond({
                            content:
                                '<:icons_Wrong:947468536492752906> Du kannst jemandem maximal 10000 Einladungen und minimal 1 hinzufügen! <:icons_Wrong:947468536492752906>',
                            ephemeral: true,
                        });
                    }
                }
...
export async function bonusRemoveInvites(i: harmony.Interaction) {
    try {
        if (i.member) {
            if (!(await isAuthorized(i.member))) {
                await i.respond({
            ...
            if (i.isApplicationCommand() && i.guild) {
                if (i.option<string>('anzahl') != undefined) {
                    ...
                    if (anzahl < 10000 && anzahl > 0) {
                        ...
                        await i.respond({
                            embeds: [{
                                'title':
                                    '<:icons_Correct:947467655630164038> Erfolgreich! <:icons_Correct:947467655630164038>',
                                'description':
                                    `Du hast ${member.user.username} **${anzahl}** Einladungen entfernt.`,
                                'color': 15658734,
                                'author': {
                                    'name': 'Einladungs-Tracker',
                                    'icon_url':
                        ...
                        await i.respond({
                            content:
                                '<:icons_Wrong:947468536492752906> Du kannst jemandem maximal 10000 Einladungen und minimal 1 entfernen! <:icons_Wrong:947468536492752906>',
                            ephemeral: true,
                        });
                    }
                }
