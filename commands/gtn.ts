...
import { randomNumber } from 'https://deno.land/x/random_number@v0.1.0/mod.ts';
import { noPerms } from '../util/noPerms.ts';
export async function gtn(i: harmony.Interaction, client: harmony.Client) {
    try {
        if (i.member) {
            if (i.guild) {
                const zahl = randomNumber({ integer: true, max: 100 });
                let guessed: number;
                    ...
                    embeds: [
                        {
                            ...
                            'description':
                                '*Bei Guess The Number musst du eine Zahl erraten und dir wird gesagt, ob die Lösung größer oder Kleiner ist.*\n**Benutze `/guess gtn <zahl>` um die Zahl zu erraten (0-100)**',
                            'color': 44469,
                            'footer': {
                ...
                while (true) {
                    const answer1 = await client.waitFor(
                        ...
                        (i2) => {
                            if (
                                i2.member && i.member && i2.channel && i.channel
                            ) {
                                return i2.member.id == i.member.id &&
                    ...
                    if (
                        answer instanceof harmony.Interaction &&
                        answer.isApplicationCommand() &&
                        answer.name == 'guess' && answer.subCommand == 'gtn'
                    ) {
                        ...
                        if (guessed) {
                            if (guessed < zahl) {
                                await answer.respond({
                                    content:
                                        ':game_die: Die gesuchte Zahl ist **größer** :game_die:',
                                });
                            }
                            if (guessed > zahl) {
                                await answer.respond({
                                    content:
                                        ':game_die: Die gesuchte Zahl ist **kleiner** :game_die:',
                                });
                            }
                            if (guessed == zahl) {
                                const endtime = Date.now();
                                let difference = endtime - startTime;
                                const realdifference = endtime - startTime;
                                ...
                                if (!((realdifference / 300) > 99)) {
                                    belohnung = Math.floor(
                                        100 - (realdifference / 300),
                                    );
                                }
                                await answer.respond({
                                    embeds: [
