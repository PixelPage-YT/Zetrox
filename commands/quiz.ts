...
export async function quiz(i: harmony.Interaction, client: harmony.Client) {
    try {
        if (i.member) {
            if (i.guild) {
                const emojis =
                    JSON.parse(Deno.readTextFileSync('./data/quiz.json')).data;
                ...
                if (answer instanceof harmony.Interaction) {
                    if (answer.isMessageComponent()) {
                        if (
                            (answer.customID == 'quiz-1' &&
                                solution.solution == 0) ||
                            (answer.customID == 'quiz-2' &&
                                solution.solution == 1) ||
                            (answer.customID == 'quiz-3' &&
                                solution.solution == 2) ||
                            (answer.customID == 'quiz-4' &&
                                solution.solution == 3)
                        ) {
                            // Richtig
                            ...
                            answer.respond({
                                content:
                                    '<:icons_Wrong:947468536492752906> Deine Antwort war Falsch! <:icons_Wrong:947468536492752906>',
                            });
                        }
                    }
                } else {
