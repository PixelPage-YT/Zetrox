...
export async function eAntiSpamTime(i: harmony.Interaction) {
    try {
            ...
            if (i.member) {
                if (i.guild) {
                    if (i.option<string>('time') != undefined) {
                        const time = parseInt(i.option<string>('time'));
                        if (time > -1 && time < 100) {
                                ...
                                embeds: [
                                    {
                                        ...
                                        'description':
                                            `Du hast die Einstellung \`AntiSpamTime\` auf ${time.toString()} Sekunden gesetzt!`,
                                        'color': 44469,
                                        'footer': {
                                            'text':
                            ...
                            await i.respond({
                                content:
                                    '<:icons_Wrong:947468536492752906> Die Zeit in Sekunden muss über 0 und unter 100 Sekunden sein! <:icons_Wrong:947468536492752906>',
                                ephemeral: true,
                            });
                        }
                    }
