    ...
    if (Deno.args[1] != undefined) {
        if (Deno.args[1] == 'newSlash') {
            if (Deno.args[0] != undefined && Deno.args[0] == 'prod') {
                const customCommand = Deno.args[2];
                if (customCommand != undefined) {
                    console.log(
                        '[Info] Slash Command ' + customCommand +
                            ' wird gelöscht',
                    );
                    for (
                        const command
                            of (await client.interactions.commands.all())
                                .array()
                    ) {
                ...
                if (customCommand != undefined) {
                    ...
                    for (const guild of clguilds) {
                        const commands = await guild.commands.all();
                        for (const command of commands.array()) {
                            if (command.name == customCommand) {
                                command.delete();
                                console.info(
                                    '[Info] Slash Command ' + command.name +
                                        ' in dem Server ' +
                                        command.guild?.name + ' gelöscht.',
                                );
                            }
                        }
                    }
                    ...
                    for (const guild of clguilds) {
                        ...
                        for (const command of commands.array()) {
                            ...
                            console.info(
                                '[Info] Slash Command ' + command.name +
                                    ' in dem Server ' + command.guild?.name +
                                    ' gelöscht.',
                            );
                        }
                    }
            ...
            client.guilds.array().then((guilds) => {
                guilds.forEach((guild) => {
                    guild.commands.all().then((existingcommands) => {
                        const excommandarray = existingcommands.array();
                        let check = false;
                        for (const i in excommandarray) {
                        ...
                        if (check == false) {
                            client.interactions.commands.create(
                                ...
                                .then((cmd) =>
                                    console.log(
                                        `[Info] Slash Command ${cmd.name} in ${guild.name} erstellt`,
                                    )
                                )
                                .catch((err) => {
                                    if (
                                        ...
                                        console.log(
                                            `[Info] Es ist fehlgeschlagen, den Slash Command ${command.name} in ${guild.name} einzurichten, weil: ${err}`,
                                        );
                                    }
                                });
                        }
    ...
    if (Deno.args[0] != undefined && Deno.args[0] == 'prod') {
        commands.forEach((command) => {
            client.interactions.commands.create(command)
                .then((cmd) =>
                    console.log(`[Info] Slash Command ${cmd.name} erstellt`)
                )
                .catch((err) =>
                    console.log(
                        `[Info] Es ist fehlgeschlagen, den Slash Command ${command.name} einzurichten, weil: ${err}`,
                    )
                );
        });
    } else {
