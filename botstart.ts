const config: { startcmd: string; token: string; watch: string } = JSON.parse(
    Deno.readTextFileSync('botstart.config.json'),
);
...
client.on('presenceUpdate', async (presence) => {
    if (presence.status == 'offline' && presence.user.id == config.watch) {
        ...
        if (logchannel.isText()) {
            logchannel.send(
                'Zetrox ist abgestürzt und wird neu gestartet. Fehler: ' +
                    new TextDecoder().decode(await latestpr.stderrOutput()),
            );
        }
    }
});
client.connect(config.token, [
    ...
    harmony.GatewayIntents.GUILD_PRESENCES,
]);
